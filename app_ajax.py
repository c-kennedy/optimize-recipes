...
def search():
    ...
    text_to_search = request.form['text'].lower()
    condition = sc.info_df.name_lower.str.contains(text_to_search)
...
def generate():
    ...
    for i in [1, 2]:
        try:
            recipe_selections = [int(n) for n in data['selector{}'.format(i)]]
        except KeyError:
            continue
...
class FormattedPricedRecipe(object):
    ...
    def display(self, n_servings=None):
        ...
        result['store'] = 'Giant Eagle' # TODO: make this actually do something!
        return result
